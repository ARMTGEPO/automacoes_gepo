...
def validar_e_gerar_relatorio(df, output_path="relatorio_inconsistencias.pdf"):
    inconsistencias = {}
    hoje = pd.to_datetime(datetime.today().replace(day=1))
    primeiro_dia_mes_anterior = hoje - pd.DateOffset(months=1)
    ...
    mask3 = df.groupby(["CPF do Aluno", "Turma"])["Matrícula"].transform('nunique') > 1
    inconsistencias['CPF com Múltiplas Matrículas na mesma turma'] = df.loc[mask3, [
    ...
    for tipo, df_inc in inconsistencias.items():
        if not df_inc.empty:
            ...
            for col, width in zip(df_inc.columns, col_widths):
                pdf.cell(width, 6, col, border=1, align='C', fill=True)
            pdf.ln()
...
# === Interface Principal ===
st.title("📝 Fechamento de Produção")
st.markdown(f"Olá, **{st.session_state['username']}**! Faça upload do Relatório Analítico Mensal de Produção.")
...
if uploaded_file:
    try:
        ...
        df["Termino da Execução da Turma"] = pd.to_datetime(df["Termino da Execução da Turma"], dayfirst=True)

        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
            validar_e_gerar_relatorio(df, tmp.name)
